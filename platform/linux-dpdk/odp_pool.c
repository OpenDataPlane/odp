...
{
	struct rte_mbuf *mbuf = &buf_hdl_to_hdr(buf)->mb;
...
void odp_buffer_free_multi(const odp_buffer_t buf[], int num)
{
	...
	for (i = 0; i < num; i++) {
		struct rte_mbuf *mbuf = &buf_hdl_to_hdr(buf[i])->mb;
...
int odp_pool_info(odp_pool_t pool_hdl, odp_pool_info_t *info)
{
	...
	if (pool->s.params.type == ODP_POOL_PACKET)
		info->pkt.max_num = rte_mempool_avail_count(pool->s.rte_mempool);
