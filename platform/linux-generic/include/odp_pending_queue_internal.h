...
typedef void (*_odp_pending_free_fn_t)(odp_event_t event);

typedef struct {
	odp_event_t event;
	odp_queue_t queue;
} _odp_pending_event_t;
...
typedef struct {
	...
	_odp_pending_event_t *ring;	/* Ring buffer of pending events */
	_odp_pending_free_fn_t free_fn;	/* Cleanup callback or NULL */
} _odp_pending_queue_t;
 ...
 */
void _odp_pending_queue_init(_odp_pending_queue_t *pending, uint32_t max_pending,
			     _odp_pending_free_fn_t free_fn, void *ring_addr);

void _odp_pending_queue_destroy(_odp_pending_queue_t *pending);

static inline int _odp_pending_queue_is_empty(_odp_pending_queue_t *pending)
{
 ...
 * 'queues' array.
 */
void _odp_pending_queue_defer(_odp_pending_queue_t *pending,
 ...
 */
uint32_t _odp_pending_queue_retry(_odp_pending_queue_t *pending);
