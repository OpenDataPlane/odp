			...
			uint32_t *offset, uint32_t frame_len)
{
	...
	macaddr0 = odp_be_to_cpu_16(*((const odp_una_u16_t *)(const void *)eth));
	if (odp_unlikely((macaddr0 & 0x0100) == 0x0100))
		input_flags.eth_mcast = 1;

	if (odp_unlikely(macaddr0 == 0xffff)) {
		macaddr2 =
			odp_be_to_cpu_16(*((const odp_una_u16_t *)
					    (const void *)eth + 1));
		macaddr4 =
			odp_be_to_cpu_16(*((const odp_una_u16_t *)
					    (const void *)eth + 2));
	...
	*parseptr += sizeof(*eth);
	...
	if (odp_unlikely(ethtype < _ODP_ETH_LEN_MAX)) {
		...
		ethtype = odp_be_to_cpu_16(*((const odp_una_u16_t *)(uintptr_t)
					      (*parseptr + 6)));
		*offset   += 8;
		*parseptr += 8;
	}
