...
/* Length in bytes */
#define _ARM_CRYPTO_MAX_CIPHER_KEY_LENGTH      16
				    ...
				    odp_crypto_generic_session_t *session)
{
	...
	/* To Do: Preallocated buffer can be used */
	output = (uint8_t *)malloc(len);

	/* Dummy auth data to feed cipher function. */
	auth = (uint8_t *)malloc(len);

	if (!output || !auth) {
		ODP_DBG("ARM Crypto: Memory allocation failed\n");
		return -1;
	}
	...
	/*  Dummy auth key to avoid crash */
	arg.digest.hmac.key = session->cipher.key_data;
	arg.digest.hmac.i_key_pad = session->cipher.key_data;
	arg.digest.hmac.o_key_pad = session->cipher.key_data;

	if (armv8_enc_aes_cbc_sha1_128(data, output, clen, data, auth, alen, &arg) != 0) {
		ODP_DBG("ARM Crypto: Encoding failed\n");
		return -1;
	}
	/* To Do: Reserve and recycle output buffer instead of malloc/free */
	odp_packet_copy_from_mem(pkt, offset, len, output);
				    ...
				    odp_crypto_generic_session_t *session)
{
	...
	if (armv8_dec_aes_cbc_sha1_128(data, output, clen, data, auth, alen, &arg) != 0) {
		ODP_DBG("ARM Crypto: Decoding failed\n");
		return -1;
	}
	/* To Do: Reserve and recycle output buffer instead of malloc/free */
	odp_packet_copy_from_mem(pkt, offset, len, output);
...
static int process_cipher_param(odp_crypto_generic_session_t *session)
{
	...
	if (_ARM_CRYPTO_MAX_CIPHER_KEY_LENGTH !=
			session->p.cipher_key.length)
		return -1;

	/* Verify IV len is correct */
	if (_ARM_CRYPTO_MAX_IV_LENGTH !=
...
static
odp_crypto_alg_err_t auth_digest_gen(odp_packet_t pkt ODP_UNUSED,
				     const odp_crypto_packet_op_param_t *param ODP_UNUSED,
				     odp_crypto_generic_session_t *session ODP_UNUSED)
{
	return ODP_CRYPTO_ALG_ERR_NONE;
...
static
odp_crypto_alg_err_t auth_digest_check(odp_packet_t pkt ODP_UNUSED,
			  ...
			  odp_crypto_ses_create_err_t *status)
{
	...
	case ODP_CIPHER_ALG_NULL:
		...
		break;
	case ODP_CIPHER_ALG_AES_CBC:
		/* Only 128b key length is supported */
	...
	case ODP_AUTH_ALG_NULL:
		...
		break;
	case ODP_AUTH_ALG_SHA1:
		rc = process_digest_param(session);
		break;
	default:
		rc = -1;
	}
