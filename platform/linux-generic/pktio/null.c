...
static int null_close(pktio_entry_t *pktio_entry)
{
...
{
	pktio_ops_null_data_t *_data = NULL;

	if (strncmp(devname, "null:", 5) != 0)
		return -1;

	pktio_entry->s.ops_data = ODP_OPS_DATA_ALLOC(sizeof(*_data));
	if (odp_unlikely(pktio_entry->s.ops_data == NULL)) {
		ODP_ERR("Failed to allocate pktio_ops_null_data_t struct");
		return -1;
	}

	_data = pktio_entry->s.ops_data;
...
{
	pktio_ops_null_data_t *_data = pktio_entry->s.ops_data;

	_data->promisc = !!enable;
	return 0;
}
