...
odp_cos_t odp_cls_cos_create(const char *name, const odp_cls_cos_param_t *param_in)
{
	...
	for (i = 0; i < CLS_COS_MAX_ENTRY; i++) {
		...
		if (0 == cos->valid) {
			...
			cos->index = i;
			if (param.vector.enable) {
				cos->vector.enabled = true;
				cos->vector.type = ODP_EVENT_PACKET_VECTOR;
				cos->vector.pool = param.vector.pool;
				cos->vector.max_size = param.vector.max_size;
			}
			if (event_aggr_enabled) {
				cos->vector.enabled = true;
				cos->vector.type = ODP_EVENT_VECTOR;
				cos->vector.max_size = param.num_queue == 1 ?
							_odp_event_aggr_max_size(param.queue) :
							param.queue_param.aggr[0].max_size;
			}
			cos->vector.dst_is_aggr = event_aggr_enabled && param.num_queue == 1;
