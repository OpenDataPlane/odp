...
static int schedule_init_global(void)
{
	...
	sched->config_if.num_group_prios = NUM_SCHED_GRPS * NUM_PRIO;
	...
	for (i = 0; i < NUM_SCHED_GRPS; i++) {
		...
		for (j = 0; j < (int)sched->config_if.num_prios; ++j)
			sched->sched_grp[i].level[j] = sched->config_if.min_prio + j;

		sched->sched_grp[i].num_prio = sched->config_if.num_prios;
	}
...
static void schedule_config_init(odp_schedule_config_t *config)
{
	...
	config->num_group_prios = sched->config_if.num_group_prios;
...
static int schedule_config(const odp_schedule_config_t *config)
{
	...
	if (config->num_group_prios > sched->config_if.num_group_prios) {
	...
	sched->config_if.num_group_prios = config->num_group_prios;
	...
	for (int i = SCHED_GROUP_NAMED; i < NUM_SCHED_GRPS; i++) {
		for (uint32_t j = 0; j < sched->config_if.num_prios; ++j)
			sched->sched_grp[i].level[j] = sched->config_if.min_prio + j;
...
static odp_schedule_group_t allocate_group(const char *name, const odp_thrmask_t *mask,
					   int *thr_tbl, uint32_t num_prios, int num_thr)
{
	...
	if (sched->num_grp_prios + num_prios > sched->config_if.num_group_prios) {
		_ODP_ERR("Insufficient group priorities (attempted: %u, left: %u)\n",
			 num_prios, sched->config_if.num_group_prios - sched->num_grp_prios);
		return group;
	}
						  ...
						  const odp_thrmask_t *mask)
{
	...
	odp_ticketlock_lock(&sched->grp_lock);
	group = allocate_group(name, mask, thr_tbl, sched->config_if.num_prios, count);
	odp_ticketlock_unlock(&sched->grp_lock);
						    ...
						    const odp_schedule_group_param_t *param)
{
	...
	group = allocate_group(name, mask, thr_tbl,
			       param->prio.num > 0 ? param->prio.num : sched->config_if.num_prios,
			       count);
...
static int schedule_capability(odp_schedule_capability_t *capa)
{
	...
	capa->max_group_prios = sched->config_if.num_group_prios;
