...
static int schedule_group_min_prio(odp_schedule_group_t group)
{
	...
	if (group >= 0 && group < NUM_SCHED_GRPS && sched->sched_grp[group].allocated)
...
static int schedule_init_global(void)
{
	...
	sched->config_if.num_groups = NUM_SCHED_GRPS;
...
static int schedule_config(const odp_schedule_config_t *config)
{
	...
	sched->config_if.max_prio = sched->config_if.min_prio + sched->config_if.num_prios - 1;
						    ...
						    const odp_schedule_group_param_t *param)
{
	...
	if (param->prio.min < sched->config_if.min_prio ||
	    (param->prio.min + (int)param->prio.num - 1) > sched->config_if.max_prio) {
