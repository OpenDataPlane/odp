...
typedef struct {
	 ...
	 * than timers from lower priority pools. The prioritization algorithm is implementation
	 * specific. Note that lower priority timers may get delayed due to higher priority timer
	 * processing. All values between zero and max_priority (see odp_timer_capability_t) are
	 * valid. The default value is zero.
	 */
	uint16_t priority;
