				  ...
				  int multi, int res)
{
	...
	for (i = 0; i < num; i++) {
		...
		src_seg[i].cache_stash_l3 = 1;
		src_seg[i].cache_stash_hints.l3.offset = 0;
		src_seg[i].cache_stash_hints.l3.len = ODP_CACHE_LINE_SIZE;
		dst_seg[i].addr = dst + offset;
		dst_seg[i].len  = cur_len;
		dst_seg[i].cache_stash_l3 = 1;
		dst_seg[i].cache_stash_hints.l3.offset = 0;
		dst_seg[i].cache_stash_hints.l3.len = ODP_CACHE_LINE_SIZE;
		offset += cur_len;
	}
...
static void test_dma_pkt_to_pkt(odp_dma_compl_mode_t compl_mode_mask, int multi)
{
	...
	src_seg.offset = OFFSET;
	src_seg.len    = len;
	src_seg.cache_stash_l3 = 1;
	src_seg.cache_stash_hints.l3.offset = 0;
	src_seg.cache_stash_hints.l3.len = ODP_CACHE_LINE_SIZE;
	dst_seg.packet = pkt_2;
	dst_seg.offset = OFFSET;
	dst_seg.len    = len;
	dst_seg.cache_stash_l3 = 1;
	dst_seg.cache_stash_hints.l3.offset = 0;
	dst_seg.cache_stash_hints.l3.len = ODP_CACHE_LINE_SIZE;
		       ...
		       odp_bool_t use_idx)
{
	...
	for (uint32_t i = 0; i < num; i++) {
		...
		if (use_idx) {
			seg->pkt_index = 0;
			seg->pkt_len = len;
			seg->cache_stash_l3 = 1;
			seg->cache_stash_hints.l3.offset = 0;
			seg->cache_stash_hints.l3.len = ODP_CACHE_LINE_SIZE;
		} else {
			seg->packet = pkt;
		}
