...
static void configure_cls_pmr_chain_create_saddr_pmr(int src, const char *saddr)
{
	uint32_t addr;
	uint32_t mask;
	...
	odp_cls_pmr_create(&pmr_param, 1, cos_list[CLS_DEFAULT],
			   cos_list[src]);
	CU_ASSERT_FATAL(pmr_list[src] != ODP_PMR_INVALID);
}
			     ...
			     uint16_t port, odp_bool_t saddr_first)
{
	...
	if (saddr_first) {
		configure_cls_pmr_chain_create_saddr_pmr(src, saddr);
