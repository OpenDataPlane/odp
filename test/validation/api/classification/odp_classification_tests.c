...
static int classification_suite_common_init(vector_mode_t vector_mode)
{
	...
	if (vector_mode == VECTOR_MODE_EVENT) {
		...
		if (!global.event_aggr.enabled) {
			printf("Event vector mode is not supported. Test suite skipped.\n");
		} else {
...
static int check_evv_queue_stats(void)
{
	if (!global.event_aggr.enabled)
		return ODP_TEST_INACTIVE;
	return check_queue_stats();
...
odp_testinfo_t classification_suite_evv[] = {
	...
	ODP_TEST_INFO_CONDITIONAL(cls_pktio_test_evv, check_evv),
	ODP_TEST_INFO_CONDITIONAL(cls_queue_stats_evv, check_evv_queue_stats),
	ODP_TEST_INFO_NULL,
};
