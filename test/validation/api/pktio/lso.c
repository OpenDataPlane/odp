					...
					odp_packet_t pkt, uint16_t seg_num, uint16_t seg_offset))
{
	...
	for (i = 0; i < num; i++) {
		...
		if (num > 1) {
			/* Segmentation occurred, update expected header */
			update_hdr(expected_hdr, hdr_len, pkt_len, pkt_out[i], seg_num,
				   offset - hdr_len);
		}
...
static void update_custom_hdr_segnum(uint8_t *hdr, uint32_t hdr_len, uint32_t orig_pkt_len,
				     odp_packet_t pkt, uint16_t seg_num, uint16_t seg_offset)
{
	(void)orig_pkt_len;
	(void)pkt;
...
static void update_custom_hdr_bits(uint8_t *hdr, uint32_t hdr_len, uint32_t orig_pkt_len,
				   odp_packet_t pkt, uint16_t seg_num, uint16_t seg_offset)
{
	(void)hdr_len;
	uint8_t value;
	...
	} else if (odp_packet_len(pkt) + seg_offset < orig_pkt_len) {
		value = LSO_TEST_MIDDLE_SEG_VALUE & LSO_TEST_MIDDLE_SEG_MASK;
		hdr[LSO_TEST_CUSTOM_ETH_BITS_OFFSET] &= ~LSO_TEST_MIDDLE_SEG_MASK;
		hdr[LSO_TEST_CUSTOM_ETH_BITS_OFFSET] |= LSO_TEST_MIDDLE_SEG_VALUE;
	} else {
		value = LSO_TEST_LAST_SEG_VALUE & LSO_TEST_LAST_SEG_MASK;
		hdr[LSO_TEST_CUSTOM_ETH_BITS_OFFSET] &= ~LSO_TEST_LAST_SEG_MASK;
		hdr[LSO_TEST_CUSTOM_ETH_BITS_OFFSET] |= LSO_TEST_LAST_SEG_VALUE;
	}
}

static void update_custom_hdr_segnum_bits(uint8_t *hdr, uint32_t hdr_len, uint32_t orig_pkt_len,
					  odp_packet_t pkt, uint16_t seg_num, uint16_t seg_offset)
{
	update_custom_hdr_segnum(hdr, hdr_len, orig_pkt_len, pkt, seg_num, seg_offset);
	update_custom_hdr_bits(hdr, hdr_len, orig_pkt_len, pkt, seg_num, seg_offset);
}
