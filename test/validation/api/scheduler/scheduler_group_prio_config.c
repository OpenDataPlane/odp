...
static void schedule_config_limits_max_prios(void)
{
	...
	CU_ASSERT(max_prio == min_prio + (int)test_info.num_prios - 1);
...
static void schedule_group_param_limits_group_prios(void)
{
	...
	int min_prio, max_prio, def_prio, num_prio;
	...
	min_prio = odp_schedule_group_min_prio(group);
	...
	CU_ASSERT(max_prio == min_prio + (int)test_info.num_g_prios - 1);
	CU_ASSERT(def_prio >= min_prio && def_prio <= max_prio);
}
