...
{
	int idx = *(int *)arg;
	odp_schedule_group_t private_group = globals->pgt_groups[idx];
	odp_queue_t private_queue = globals->pgt_queues[idx];
	odp_event_t ev;
	odp_queue_t from;
	odp_thrmask_t mask;
	uint32_t i = 0;
...
static void scheduler_test_private_groups(void)
{
	...
	int idx[MAX_WORKERS];
	void *args[MAX_WORKERS];
	uint32_t num_predef = 3; /* Predefined groups */
