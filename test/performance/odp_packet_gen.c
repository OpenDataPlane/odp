			  ...
			  test_options_t *opt, odp_pool_t pool)
{
	...
	if (opt->lso.param.lso_proto == ODP_LSO_PROTO_TCP_IPV4 &&
	    !pktio_capa->lso.proto.tcp_ipv4) {
		ODPH_ERR("Error (%s): ODP_LSO_PROTO_TCP_IPV4 not supported\n", name);
		return -1;
	} else if (opt->lso.param.lso_proto == ODP_LSO_PROTO_IPV4 &&
		   !pktio_capa->lso.proto.ipv4) {
		ODPH_ERR("Error (%s): ODP_LSO_PROTO_IPV4 not supported\n", name);
		return -1;
	} else if (opt->lso.param.lso_proto == ODP_LSO_PROTO_CUSTOM &&
		   !pktio_capa->lso.proto.custom) {
