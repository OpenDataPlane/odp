...
typedef struct {
	...
	uint32_t num_evv;        /** Number of events in each event vector */
} appl_args_t;
...
static void allocate_test_evv_and_contents(odp_event_vector_t evv[], int num)
{
	...
	uint32_t num_evv = gbl_args->appl.num_evv;
...
static void free_event_vectors(void)
{
	...
	for (int i = 0; i < TEST_REPEAT_COUNT; i++) {
		evv_size = odp_event_vector_size(evv_arr[i]);

		if (evv_size != 0 && evv_size != num_evv)
			odp_event_vector_size_set(evv_arr[i], num_evv);
...
static void free_buf_events(void)
{
	...
	for (int i = 0; i < TEST_REPEAT_COUNT; i++)
		odp_event_free_multi(buf_event_tbl[i], num_evv);
}
...
static int event_vector_alloc_free(void)
{
	...
	for (i = 0; i < TEST_REPEAT_COUNT; i++)
		odp_event_vector_free(odp_event_vector_alloc(evv_pool));
...
static int event_vector_tbl(void)
{
	...
	for (i = 0; i < TEST_REPEAT_COUNT; i++)
		odp_event_vector_tbl(evv_arr[i], &ptr);
...
static int event_vector_size(void)
{
	...
	for (i = 0; i < TEST_REPEAT_COUNT; i++)
		odp_event_vector_size(evv_arr[i]);
...
static int event_vector_size_set(void)
{
	...
	for (i = 0; i < TEST_REPEAT_COUNT; i++)
		odp_event_vector_size_set(evv_arr[i], 1);
...
static void usage(void)
{
	printf("\n"
	       ...
	       "\n", TEST_ROUNDS, TEST_MAX_EVV_SIZE, TEST_EVV_SIZE);
}
...
static void parse_args(int argc, char *argv[], appl_args_t *appl_args)
{
	...
	appl_args->num_evv = TEST_EVV_SIZE;
	appl_args->time = 0;
...
static void print_info(void)
{
	...
	printf("Event vector size:       %d\n", gbl_args->appl.max_evv_size);
	printf("Event vector elements:   %d\n", gbl_args->appl.num_evv);
...
bench_info_t test_suite[] = {
	...
	BENCH_INFO(event_vector_size_set, create_event_vectors, free_event_vectors, NULL),
...
static int create_pools(void)
{
	...
	if (capa.event_vector.max_num && capa.event_vector.max_num < TEST_REPEAT_COUNT) {
		...
		return -1;
	} else if (gbl_args->appl.cache_size > (int)capa.event_vector.max_cache_size) {
		ODPH_ERR("Error: cache size not supported (max %" PRIu32 ")\n",
			 capa.event_vector.max_cache_size);
		return -1;
	}

	if (gbl_args->appl.max_evv_size > TEST_MAX_EVV_SIZE ||
	    gbl_args->appl.max_evv_size > capa.event_vector.max_size) {
		ODPH_ERR("Error: event vector size not supported (max %" PRIu32 ")\n",
			 capa.event_vector.max_size < TEST_MAX_EVV_SIZE ?
			 capa.event_vector.max_size : TEST_MAX_EVV_SIZE);
		return -1;
	}

	if (gbl_args->appl.num_evv > gbl_args->appl.max_evv_size) {
		ODPH_ERR("Error: number of event vector elements is larger than max (%u/%u)\n",
			 gbl_args->appl.num_evv, gbl_args->appl.max_evv_size);
		return -1;
	}

	gbl_args->evv_uarea_size = TEST_UAREA_SIZE < capa.event_vector.max_uarea_size ?
				TEST_UAREA_SIZE : capa.event_vector.max_uarea_size;
	...
	/* Create buffer pool */
	if (gbl_args->appl.num_evv) {
		uint32_t buf_size = TEST_BUF_SIZE;

		odp_pool_param_init(&buf_pool_params);
		buf_pool_params.buf.size = buf_size;
		buf_pool_params.buf.num = TEST_REPEAT_COUNT * gbl_args->appl.num_evv;
