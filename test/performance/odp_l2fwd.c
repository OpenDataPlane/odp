...
static inline void handle_tx_poll_compl(tx_compl_t *tx_c, odp_packet_t pkts[], int num, int tx_idx,
					stats_t *stats)
{
	uint32_t poll_head = tx_c->poll_head, free_head = tx_c->free_head, next_free;
	const uint32_t max = tx_c->max, init = tx_c->init;
	odp_pktio_t pktio = gbl_args->pktios[tx_idx].pktio;
	int next_req = tx_c->next_req;
	odp_packet_t pkt;
	const int interval = tx_c->interval;

	while (poll_head != free_head) {
	...
	while (next_req <= num) {
		pkt = pkts[next_req - 1];
		next_free = free_head + 1;

		if (next_free > max)
			next_free = init;

		if (next_free == poll_head) {
			stats->s.tx_c_misses++;
			/* Missed one, try requesting for the first packet of next burst. */
			next_req = num + 1;
			break;
