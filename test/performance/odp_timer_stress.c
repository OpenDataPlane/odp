...
static void init_config(prog_config_t *config)
{
	...
	for (uint32_t i = 0U; i < MAX_WORKERS; ++i) {
		...
		worker->cancel.shm = ODP_SHM_INVALID;
	}
...
static parse_result_t check_options(prog_config_t *config)
{
	...
	int ret;
	uint32_t req_tmr, max_pools, max_workers, req_shm;
	odp_fract_u64_t hz;
	...
	if (opts->mode == SINGLE_SHOT || opts->mode == CANCEL) {
		...
		if (opts->mode == CANCEL) {
			...
			/* One block for program configuration, one for each worker. */
			req_shm = opts->num_workers + 1U;
			...
			req_shm_sz = sizeof(tmr_hdls_t) * opts->num_tmr;
		...
		max_pools = tmr_capa.periodic.max_pools;
	}

	if (max_pools == 0U) {
		ODPH_ERR("Invalid timer type support: %u\n", opts->mode);
		return PRS_NOK;
	}
...
static odp_bool_t setup_config(prog_config_t *config)
{
	...
	for (uint32_t i = 0U; i < opts->num_workers; ++i) {
		...
		if (is_priv) {
			if (opts->mode == CANCEL) {
				worker->cancel.shm = odp_shm_reserve(PROG_NAME,
								     worker->num_boot_tmr *
								     sizeof(tmr_hdls_t),
								     ODP_CACHE_LINE_SIZE, 0U);

				if (worker->cancel.shm == ODP_SHM_INVALID) {
					ODPH_ERR("Error reserving SHM for cancel mode\n");
					return false;
				}

				worker->cancel.tmrs = odp_shm_addr(worker->cancel.shm);

				if (worker->cancel.tmrs == NULL) {
...
static int process_single_shot(void *args)
{
	...
	while (true) {
		ev = odp_schedule(NULL, odp_schedule_wait_time(stats->max_mul * res_ns * 2U));
...
static int process_cancel(void *args)
{
	...
	while (odp_atomic_load_u32(is_running)) {
		...
		for (uint32_t i = 0U; i < num_boot_tmr; ++i) {
			...
			int ret = odp_timer_cancel(time->tmr, &ev);
...
static void print_stats(const prog_config_t *config)
{
	...
	for (uint32_t i = 0U; i < config->opts.num_workers; ++i) {
		...
		printf("    runtime:            %" PRIu64 " ns\n"
		       "    rate:               ", stats->tot_tm);
		print_humanised(stats->num_tmo / ((double)stats->tot_tm / ODP_TIME_SEC_IN_NS));
		printf("\n");
	}
...
static void teardown(const prog_config_t *config)
{
	...
	for (uint32_t i = 0U; i < opts->num_workers; ++i) {
		...
		if (worker->cancel.shm != ODP_SHM_INVALID)
			(void)odp_shm_free(worker->cancel.shm);
	}
