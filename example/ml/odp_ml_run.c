...
static void calc_io_size(void)
{
	for (int i = 0; i < glb.num_inp; i++) {
		...
		glb.inp[i].elems = elems;
		glb.inp[i].elem_size = info->data_type_size;
		glb.inp[i].size = elems * info->data_type_size;
		glb.inp_size_q += glb.inp[i].size;
		glb.inp_size_d += elems * sizeof(float);

		printf(", elems: %" PRIu64 ", datatype size: %d, size: %" PRIu64 "\n",
		       glb.inp[i].elems, glb.inp[i].elem_size, glb.inp[i].size);
	}
	...
	for (int i = 0; i < glb.num_out; i++) {
		...
		if (shape->type == ODP_ML_SHAPE_BATCH)
			elems *= glb.opt.num_batch;
		glb.out[i].elems = elems;
		glb.out[i].elem_size = info->data_type_size;
		glb.out[i].size = elems * info->data_type_size;
		glb.out_size_q += glb.out[i].size;
		glb.out_size_d += elems * sizeof(float);

		printf(", elems: %" PRIu64 ", datatype size: %d, size: %" PRIu64 "\n",
		       glb.out[i].elems, glb.out[i].elem_size, glb.out[i].size);
	}
